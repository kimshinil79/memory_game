...
def add_by_country_translation(file_path):
    ...
    try:
        ...
        # Find the last entry before the closing }; and add by_country
        ...
        if match:
            ...
            # Try a simpler approach - find the last 'key': 'value', and add before it
            pattern2 = r"(\s+)'([^']+)':\s*'([^']+)',\s*\n(\s+)\};"
            match2 = re.search(pattern2, content, re.MULTILINE)
            
            if match2:
                indent = match2.group(1)
                last_key = match2.group(2)
                last_value = match2.group(3)
                closing_indent = match2.group(4)
                
                # Add by_country entry before the last entry
                new_entry = f"{indent}'by_country': '{translation}',\n{indent}'{last_key}': '{last_value}',\n{closing_indent}}};"
                new_content = re.sub(pattern2, new_entry, content, flags=re.MULTILINE)
