// 벨기에 번역 파일

// Belgian translations (Dutch, French, German)
const Map<String, String> nlBETranslations = {
  'select_language': 'Selecteer taal',
  'search_language': 'Zoek taal',
  'all': 'Alle',
  'asian_languages': 'Aziatische talen',
  'european_languages': 'Europese talen',
  'middle_eastern_languages': 'Midden-Oosterse talen',
  'african_languages': 'Afrikaanse talen',
  'cancel': 'Annuleren',
  'ok': 'OK',
  'yes': 'Ja',
  'no': 'Nee',
  'save': 'Opslaan',
  'app_title': 'Geheugenspel',
  'delete': 'Verwijderen',
  'edit': 'Bewerken',
  'close': 'Sluiten',
  'back': 'Terug',
  'next': 'Volgende',
  'continue': 'Doorgaan',
  'settings': 'Instellingen',
  'profile': 'Profiel',
  'home': 'Home',
  'game': 'Spel',
  'ranking': 'Ranglijst',
  'brain_health': 'Hersengezondheid',
  'player': 'Speler',
  'players': 'Spelers',

  // Player Selection Dialog texts
  'select_players': 'Selecteer spelers',
  'select_up_to_3_players': 'Selecteer maximaal 3 spelers',
  'you_will_be_included': 'Je wordt altijd als speler meegerekend',
  'confirm': 'Bevestigen',
  'retry': 'Opnieuw proberen',
  'no_other_users': 'Geen andere gebruikers gevonden',
  'failed_to_load_users': 'Kon gebruikerslijst niet laden',
  'country': 'Land',
  'level': 'Niveau',
  'unknown': 'Onbekend',
  'unknown_player': 'Onbekende speler',
  'multiplayer_verification': 'Multiplayer verificatie',
  'create_pin': 'PIN aanmaken',
  'enter_pin_for': 'Voer PIN in voor',
  'no_pin_for': 'Geen PIN voor',
  'create_pin_for_multiplayer': 'Maak een 2-cijferige PIN voor multiplayer',
  'enter_2_digit_pin': 'Voer 2-cijferige PIN in',
  'pin_is_2_digits': 'PIN moet 2 cijfers zijn',
  'wrong_pin': 'Verkeerde PIN',

  // Grid Selection Dialog texts
  'select_grid_size': 'Selecteer rastergrootte',
  'choose_difficulty': 'Kies moeilijkheidsgraad',
  'multiplier': '×',

  // Profile Edit Dialog texts
  'edit_profile': 'Profiel bewerken',
  'nickname': 'Bijnaam',
  'enter_nickname': 'Voer bijnaam in',
  'birthday': 'Geboortedatum',
  'select_birthday': 'Selecteer geboortedatum',
  'gender': 'Geslacht',
  'male': 'Man',
  'female': 'Vrouw',
  'select_country': 'Selecteer land',
  'multi_game_pin': 'Multiplayer PIN',
  'enter_two_digit_pin': 'Voer 2-cijferige PIN in',
  'two_digit_pin_helper': 'Deze PIN wordt gebruikt voor multiplayer sessies',
  'change_password': 'Wachtwoord wijzigen',
  'current_password': 'Huidig wachtwoord',
  'enter_current_password': 'Voer huidig wachtwoord in',
  'new_password': 'Nieuw wachtwoord',
  'enter_new_password': 'Voer nieuw wachtwoord in',
  'confirm_password': 'Bevestig wachtwoord',
  'confirm_new_password': 'Bevestig nieuw wachtwoord',
  'must_be_two_digit': 'Moet 2 cijfers zijn',
  'current_password_required': 'Huidig wachtwoord vereist',
  'password_length_error': 'Wachtwoord moet minimaal 6 tekens zijn',
  'passwords_do_not_match': 'Wachtwoorden komen niet overeen',
  'incorrect_current_password': 'Onjuist huidig wachtwoord',
  'error_changing_password': 'Fout bij wijzigen wachtwoord',
  'error': 'Fout',
  'sign_out': 'Uitloggen',
  'random_shake': 'Schud de kaarten!!',

  // Completion Dialog texts
  'congratulations': 'Gefeliciteerd!',
  'winner': 'Winnaar: {name}!',
  'its_a_tie': 'Gelijkspel!',
  'points_divided': 'Punten worden gedeeld bij gelijkspel!',
  'time_seconds': 'Tijd: {seconds} seconden',
  'flips': 'Omgedraaid: {count}',
  'players_score_multiplier': '({players} spelers: punten ×{multiplier})',
  'points_divided_explanation': '(Punten worden gedeeld bij gelijkspel)',
  'health_score': 'Hersengezondheid score: +{points}',
  'new_game': 'Nieuw spel',
  'times_up': 'Tijd is om!',

  // Tutorial Overlay texts
  'memory_game_guide': 'Geheugenspel handleiding',
  'card_selection_title': 'Kaartselectie',
  'card_selection_desc': 'Klik op kaarten om paren te vinden.',
  'time_limit_title': 'Tijdslimiet',
  'time_limit_desc':
      'Vind alle paren voordat de tijd om is. Snelle paren leveren meer punten op.',
  'add_time_title': 'Tijd toevoegen',
  'add_time_desc':
      'Klik op "+30s" om tijd toe te voegen (kost hersengezondheid punten).',
  'multiplayer_title': 'Multiplayer',
  'multiplayer_desc':
      'Wijzig het aantal spelers (1-4) om met vrienden te spelen.',
  'dont_show_again': 'Niet meer tonen',
  'start_game': 'Spel starten',

  // Brain Health Dashboard texts
  'brain_health_dashboard': 'Hersengezondheid dashboard',
  'brain_health_index_title': 'Hersengezondheid index',
  'brain_health_index_desc':
      'Volg de verbetering van je hersengezondheid index door geheugenspellen. Hoge niveaus helpen dementie te voorkomen.',
  'activity_graph_title': 'Activiteitengrafiek',
  'activity_graph_desc':
      'Bekijk hoe je hersengezondheid index verandert in de tijd op de grafiek.',
  'ranking_system_title': 'Ranglijst systeem',
  'ranking_system_desc':
      'Vergelijk je hersengezondheid index met andere gebruikers en bekijk je positie.',
  'game_statistics_title': 'Spelstatistieken',
  'game_statistics_desc':
      'Bekijk verschillende statistieken zoals gespeelde spellen, gevonden paren en beste tijden.',
  'got_it': 'Begrepen!',

  // Brain Health Dashboard additional texts
  'play_memory_games_description':
      'Speel geheugenspellen om je hersengezondheid te verbeteren!',
  'calculating_brain_health_index': 'Hersengezondheid index berekenen...',
  'error_calculating_index': 'Fout bij berekenen hersengezondheid index',
  'age': 'Leeftijd',
  'update': 'Bijwerken',
  'points_to_next_level': '{points} punten nodig voor volgend niveau',
  'maximum_level_reached': 'Maximaal niveau bereikt',
  'index_components': 'Index componenten',
  'age_factor': 'Leeftijdsfactor',
  'recent_activity': 'Recente activiteit',
  'game_performance': 'Spelprestaties',
  'persistence_bonus': 'Volhardingsbonus',
  'inactivity_penalty': 'Inactiviteitsstraf',
  'inactivity_warning':
      'Je hebt {days} dagen niet gespeeld. Je index daalt dagelijks!',
  'loading_data': 'Gegevens laden...',
  'refresh_data': 'Gegevens vernieuwen',

  // Login Prompt texts
  'start_tracking_brain_health': 'Start hersengezondheid tracking',
  'login_prompt_desc':
      'Log in om je hersengezondheid index te volgen en vooruitgang te zien. Verbeter je cognitieve vaardigheden door geheugenspellen.',
  'sign_in': 'Inloggen',
  'create_account': 'Account aanmaken',

  // User Rankings texts
  'user_rankings': 'Gebruikersranglijst',
  'rank': 'Rang',
  'user': 'Gebruiker',
  'score': 'Score',
  'failed_to_load_rankings': 'Kon ranglijst niet laden',
  'no_ranking_data': 'Geen ranglijstgegevens',

  // Date format texts
  'today': 'Vandaag',
  'yesterday': 'Gisteren',

  // Activity Chart texts
  'brain_health_progress': 'Hersengezondheid voortgang',
  'welcome_to_brain_health': 'Welkom bij Hersengezondheid!',
  'start_playing_memory_games':
      'Begin met het spelen van geheugenspellen\nom je hersengezondheid te volgen',
  'date_range': 'Datumbereik',
  'last_7_days': 'Laatste 7 dagen',
  'last_30_days': 'Laatste 30 dagen',
  'all_time': 'Aller tijden',

  // Game Statistics texts
  'game_statistics': 'Spelstatistieken',
  'games_played': 'Gespeelde spellen',
  'matches_found': 'Gevonden paren',
  'best_times': 'Beste tijden',
  'no_records_yet': 'Nog geen records',
  'grid': 'Raster',

  // Benefits Section texts
  'benefits_of_brain_games': 'Voordelen van hersenspellen',
  'short_term_memory_improvement': 'Verbetering kortetermijngeheugen',
  'short_term_memory_desc':
      'Geheugenspellen verbeteren effectief het kortetermijngeheugen en geheugencapaciteit.',
  'cognitive_function_enhancement': 'Verbetering cognitieve functies',
  'cognitive_function_desc':
      'Regelmatige hersenactiviteit helpt cognitieve functies te beschermen en te verbeteren.',
  'response_time_reduction': 'Vermindering reactietijd',
  'response_time_desc':
      'Snelle matching verbetert reactietijd en verwerkingssnelheid.',
  'dementia_prevention': 'Dementiepreventie',
  'dementia_prevention_desc':
      'Regelmatige hersentraining helpt het risico op dementie en cognitieve achteruitgang te verminderen.',

  // Test Page texts
  'memory_test': 'Geheugentest',
  'question': 'Vraag',
  'visual_memory_test': 'Visuele geheugentest',
  'visual_memory_test_desc':
      'Test je geheugen met 10 vragen. Selecteer de afbeelding die overeenkomt met het juiste woord.',
  'audio_assistance': 'Audio-assistentie',
  'audio_assistance_desc':
      'Klik op het audio-icoon om het juiste woord te horen. Audio wordt afgespeeld in de geselecteerde taal.',
  'question_navigation': 'Vraagnavigatie',
  'question_navigation_desc':
      'Gebruik de nummerindicatoren bovenaan of pijlknoppen om tussen vragen te navigeren.',
  'select_and_submit': 'Selecteer en verzend',
  'select_and_submit_desc':
      'Selecteer een afbeelding voor elke vraag. De verzendknop verschijnt na het beantwoorden van alle vragen.',
  'results_and_progress': 'Resultaten en voortgang',
  'results_and_progress_desc':
      'Na verzending zie je je resultaten en kun je een nieuwe test starten als je wilt.',
  'how_to_play': 'Hoe te spelen',
  'start_learning': 'Start leren',
  'test_result': 'Testresultaat',
  'your_score': 'Je score',
  'new_test': 'Nieuwe test',
  'submit': 'Verzenden'
};

const Map<String, String> frBETranslations = {
  'select_language': 'Sélectionner la langue',
  'search_language': 'Rechercher une langue',
  'all': 'Toutes',
  'asian_languages': 'Langues asiatiques',
  'european_languages': 'Langues européennes',
  'middle_eastern_languages': 'Langues du Moyen-Orient',
  'african_languages': 'Langues africaines',
  'cancel': 'Annuler',
  'ok': 'OK',
  'yes': 'Oui',
  'no': 'Non',
  'save': 'Enregistrer',
  'app_title': 'Jeu de mémoire',
  'delete': 'Supprimer',
  'edit': 'Modifier',
  'close': 'Fermer',
  'back': 'Retour',
  'next': 'Suivant',
  'continue': 'Continuer',
  'settings': 'Paramètres',
  'profile': 'Profil',
  'home': 'Accueil',
  'game': 'Jeu',
  'ranking': 'Classement',
  'brain_health': 'Santé cérébrale',
  'player': 'Joueur',
  'players': 'Joueurs',

  // Player Selection Dialog texts
  'select_players': 'Sélectionner les joueurs',
  'select_up_to_3_players': 'Sélectionner jusqu\'à 3 joueurs',
  'you_will_be_included': 'Vous serez toujours inclus comme joueur',
  'confirm': 'Confirmer',
  'retry': 'Réessayer',
  'no_other_users': 'Aucun autre utilisateur trouvé',
  'failed_to_load_users': 'Impossible de charger la liste des utilisateurs',
  'country': 'Pays',
  'level': 'Niveau',
  'unknown': 'Inconnu',
  'unknown_player': 'Joueur inconnu',
  'multiplayer_verification': 'Vérification multijoueur',
  'create_pin': 'Créer un PIN',
  'enter_pin_for': 'Entrer le PIN pour',
  'no_pin_for': 'Pas de PIN pour',
  'create_pin_for_multiplayer': 'Créer un PIN à 2 chiffres pour le multijoueur',
  'enter_2_digit_pin': 'Entrer un PIN à 2 chiffres',
  'pin_is_2_digits': 'Le PIN doit être à 2 chiffres',
  'wrong_pin': 'PIN incorrect',

  // Grid Selection Dialog texts
  'select_grid_size': 'Sélectionner la taille de la grille',
  'choose_difficulty': 'Choisir la difficulté',
  'multiplier': '×',

  // Profile Edit Dialog texts
  'edit_profile': 'Modifier le profil',
  'nickname': 'Pseudonyme',
  'enter_nickname': 'Entrer un pseudonyme',
  'birthday': 'Date de naissance',
  'select_birthday': 'Sélectionner la date de naissance',
  'gender': 'Genre',
  'male': 'Homme',
  'female': 'Femme',
  'select_country': 'Sélectionner le pays',
  'multi_game_pin': 'PIN multijoueur',
  'enter_two_digit_pin': 'Entrer un PIN à 2 chiffres',
  'two_digit_pin_helper': 'Ce PIN est utilisé pour les sessions multijoueur',
  'change_password': 'Changer le mot de passe',
  'current_password': 'Mot de passe actuel',
  'enter_current_password': 'Entrer le mot de passe actuel',
  'new_password': 'Nouveau mot de passe',
  'enter_new_password': 'Entrer le nouveau mot de passe',
  'confirm_password': 'Confirmer le mot de passe',
  'confirm_new_password': 'Confirmer le nouveau mot de passe',
  'must_be_two_digit': 'Doit être à 2 chiffres',
  'current_password_required': 'Mot de passe actuel requis',
  'password_length_error':
      'Le mot de passe doit contenir au moins 6 caractères',
  'passwords_do_not_match': 'Les mots de passe ne correspondent pas',
  'incorrect_current_password': 'Mot de passe actuel incorrect',
  'error_changing_password': 'Erreur lors du changement de mot de passe',
  'error': 'Erreur',
  'sign_out': 'Déconnexion',
  'random_shake': 'Mélanger les cartes!!',

  // Completion Dialog texts
  'congratulations': 'Félicitations!',
  'winner': 'Gagnant: {name}!',
  'its_a_tie': 'Égalité!',
  'points_divided': 'Les points sont partagés en cas d\'égalité!',
  'time_seconds': 'Temps: {seconds} secondes',
  'flips': 'Retournements: {count}',
  'players_score_multiplier': '({players} joueurs: points ×{multiplier})',
  'points_divided_explanation': '(Les points sont partagés en cas d\'égalité)',
  'health_score': 'Score de santé cérébrale: +{points}',
  'new_game': 'Nouveau jeu',
  'times_up': 'Le temps est écoulé!',

  // Tutorial Overlay texts
  'memory_game_guide': 'Guide du jeu de mémoire',
  'card_selection_title': 'Sélection des cartes',
  'card_selection_desc': 'Cliquez sur les cartes pour trouver les paires.',
  'time_limit_title': 'Limite de temps',
  'time_limit_desc':
      'Trouvez toutes les paires avant la fin du temps. Les paires rapides rapportent plus de points.',
  'add_time_title': 'Ajouter du temps',
  'add_time_desc':
      'Cliquez sur "+30s" pour ajouter du temps (coûte des points de santé cérébrale).',
  'multiplayer_title': 'Multijoueur',
  'multiplayer_desc':
      'Modifiez le nombre de joueurs (1-4) pour jouer avec des amis.',
  'dont_show_again': 'Ne plus afficher',
  'start_game': 'Commencer le jeu',

  // Brain Health Dashboard texts
  'brain_health_dashboard': 'Tableau de bord de santé cérébrale',
  'brain_health_index_title': 'Indice de santé cérébrale',
  'brain_health_index_desc':
      'Suivez l\'amélioration de votre indice de santé cérébrale grâce aux jeux de mémoire. Les niveaux élevés aident à prévenir la démence.',
  'activity_graph_title': 'Graphique d\'activité',
  'activity_graph_desc':
      'Voyez comment votre indice de santé cérébrale change dans le temps sur le graphique.',
  'ranking_system_title': 'Système de classement',
  'ranking_system_desc':
      'Comparez votre indice de santé cérébrale avec d\'autres utilisateurs et vérifiez votre position.',
  'game_statistics_title': 'Statistiques du jeu',
  'game_statistics_desc':
      'Consultez diverses statistiques comme les jeux joués, les paires trouvées et les meilleurs temps.',
  'got_it': 'Compris!',

  // Brain Health Dashboard additional texts
  'play_memory_games_description':
      'Jouez à des jeux de mémoire pour améliorer votre santé cérébrale!',
  'calculating_brain_health_index': 'Calcul de l\'indice de santé cérébrale...',
  'error_calculating_index':
      'Erreur lors du calcul de l\'indice de santé cérébrale',
  'age': 'Âge',
  'update': 'Mettre à jour',
  'points_to_next_level': '{points} points nécessaires pour le niveau suivant',
  'maximum_level_reached': 'Niveau maximum atteint',
  'index_components': 'Composants de l\'indice',
  'age_factor': 'Facteur d\'âge',
  'recent_activity': 'Activité récente',
  'game_performance': 'Performance du jeu',
  'persistence_bonus': 'Bonus de persévérance',
  'inactivity_penalty': 'Pénalité d\'inactivité',
  'inactivity_warning':
      'Vous n\'avez pas joué depuis {days} jours. Votre indice diminue quotidiennement!',
  'loading_data': 'Chargement des données...',
  'refresh_data': 'Actualiser les données',

  // Login Prompt texts
  'start_tracking_brain_health': 'Commencer le suivi de la santé cérébrale',
  'login_prompt_desc':
      'Connectez-vous pour suivre votre indice de santé cérébrale et voir vos progrès. Améliorez vos capacités cognitives grâce aux jeux de mémoire.',
  'sign_in': 'Connexion',
  'create_account': 'Créer un compte',

  // User Rankings texts
  'user_rankings': 'Classement des utilisateurs',
  'rank': 'Rang',
  'user': 'Utilisateur',
  'score': 'Score',
  'failed_to_load_rankings': 'Impossible de charger le classement',
  'no_ranking_data': 'Aucune donnée de classement',

  // Date format texts
  'today': 'Aujourd\'hui',
  'yesterday': 'Hier',

  // Activity Chart texts
  'brain_health_progress': 'Progrès de la santé cérébrale',
  'welcome_to_brain_health': 'Bienvenue dans Santé cérébrale!',
  'start_playing_memory_games':
      'Commencez à jouer à des jeux de mémoire\npour suivre votre santé cérébrale',
  'date_range': 'Plage de dates',
  'last_7_days': '7 derniers jours',
  'last_30_days': '30 derniers jours',
  'all_time': 'Tous les temps',

  // Game Statistics texts
  'game_statistics': 'Statistiques du jeu',
  'games_played': 'Jeux joués',
  'matches_found': 'Paires trouvées',
  'best_times': 'Meilleurs temps',
  'no_records_yet': 'Aucun record encore',
  'grid': 'Grille',

  // Benefits Section texts
  'benefits_of_brain_games': 'Avantages des jeux cérébraux',
  'short_term_memory_improvement': 'Amélioration de la mémoire à court terme',
  'short_term_memory_desc':
      'Les jeux de mémoire améliorent efficacement la mémoire à court terme et la capacité de mémoire.',
  'cognitive_function_enhancement': 'Amélioration des fonctions cognitives',
  'cognitive_function_desc':
      'L\'activité cérébrale régulière aide à protéger et améliorer les fonctions cognitives.',
  'response_time_reduction': 'Réduction du temps de réaction',
  'response_time_desc':
      'Les appariements rapides améliorent le temps de réaction et la vitesse de traitement.',
  'dementia_prevention': 'Prévention de la démence',
  'dementia_prevention_desc':
      'L\'entraînement cérébral régulier aide à réduire le risque de démence et de déclin cognitif.',

  // Test Page texts
  'memory_test': 'Test de mémoire',
  'question': 'Question',
  'visual_memory_test': 'Test de mémoire visuelle',
  'visual_memory_test_desc':
      'Testez votre mémoire avec 10 questions. Sélectionnez l\'image qui correspond au mot correct.',
  'audio_assistance': 'Assistance audio',
  'audio_assistance_desc':
      'Cliquez sur l\'icône audio pour entendre le mot correct. L\'audio est lu dans la langue sélectionnée.',
  'question_navigation': 'Navigation des questions',
  'question_navigation_desc':
      'Utilisez les indicateurs numériques en haut ou les boutons fléchés pour naviguer entre les questions.',
  'select_and_submit': 'Sélectionner et soumettre',
  'select_and_submit_desc':
      'Sélectionnez une image pour chaque question. Le bouton de soumission apparaîtra après avoir répondu à toutes les questions.',
  'results_and_progress': 'Résultats et progrès',
  'results_and_progress_desc':
      'Après soumission, vous verrez vos résultats et pourrez commencer un nouveau test si vous le souhaitez.',
  'how_to_play': 'Comment jouer',
  'start_learning': 'Commencer l\'apprentissage',
  'test_result': 'Résultat du test',
  'your_score': 'Votre score',
  'new_test': 'Nouveau test',
  'submit': 'Soumettre'
};

const Map<String, String> deBETranslations = {
  'select_language': 'Sprache auswählen',
  'search_language': 'Sprache suchen',
  'all': 'Alle',
  'asian_languages': 'Asiatische Sprachen',
  'european_languages': 'Europäische Sprachen',
  'middle_eastern_languages': 'Naher Osten Sprachen',
  'african_languages': 'Afrikanische Sprachen',
  'cancel': 'Abbrechen',
  'ok': 'OK',
  'yes': 'Ja',
  'no': 'Nein',
  'save': 'Speichern',
  'app_title': 'Gedächtnisspiel',
  'delete': 'Löschen',
  'edit': 'Bearbeiten',
  'close': 'Schließen',
  'back': 'Zurück',
  'next': 'Weiter',
  'continue': 'Fortfahren',
  'settings': 'Einstellungen',
  'profile': 'Profil',
  'home': 'Startseite',
  'game': 'Spiel',
  'ranking': 'Rangliste',
  'brain_health': 'Gehirngesundheit',
  'player': 'Spieler',
  'players': 'Spieler',

  // Player Selection Dialog texts
  'select_players': 'Spieler auswählen',
  'select_up_to_3_players': 'Bis zu 3 Spieler auswählen',
  'you_will_be_included': 'Sie werden immer als Spieler eingeschlossen',
  'confirm': 'Bestätigen',
  'retry': 'Wiederholen',
  'no_other_users': 'Keine anderen Benutzer gefunden',
  'failed_to_load_users': 'Benutzerliste konnte nicht geladen werden',
  'country': 'Land',
  'level': 'Level',
  'unknown': 'Unbekannt',
  'unknown_player': 'Unbekannter Spieler',
  'multiplayer_verification': 'Multiplayer-Verifizierung',
  'create_pin': 'PIN erstellen',
  'enter_pin_for': 'PIN eingeben für',
  'no_pin_for': 'Kein PIN für',
  'create_pin_for_multiplayer':
      'Erstellen Sie einen 2-stelligen PIN für Multiplayer',
  'enter_2_digit_pin': '2-stelligen PIN eingeben',
  'pin_is_2_digits': 'PIN muss 2-stellig sein',
  'wrong_pin': 'Falscher PIN',

  // Grid Selection Dialog texts
  'select_grid_size': 'Rastergröße auswählen',
  'choose_difficulty': 'Schwierigkeitsgrad wählen',
  'multiplier': '×',

  // Profile Edit Dialog texts
  'edit_profile': 'Profil bearbeiten',
  'nickname': 'Spitzname',
  'enter_nickname': 'Spitzname eingeben',
  'birthday': 'Geburtstag',
  'select_birthday': 'Geburtstag auswählen',
  'gender': 'Geschlecht',
  'male': 'Männlich',
  'female': 'Weiblich',
  'select_country': 'Land auswählen',
  'multi_game_pin': 'Multiplayer PIN',
  'enter_two_digit_pin': '2-stelligen PIN eingeben',
  'two_digit_pin_helper': 'Dieser PIN wird für Multiplayer-Sitzungen verwendet',
  'change_password': 'Passwort ändern',
  'current_password': 'Aktuelles Passwort',
  'enter_current_password': 'Aktuelles Passwort eingeben',
  'new_password': 'Neues Passwort',
  'enter_new_password': 'Neues Passwort eingeben',
  'confirm_password': 'Passwort bestätigen',
  'confirm_new_password': 'Neues Passwort bestätigen',
  'must_be_two_digit': 'Muss 2-stellig sein',
  'current_password_required': 'Aktuelles Passwort erforderlich',
  'password_length_error': 'Passwort muss mindestens 6 Zeichen lang sein',
  'passwords_do_not_match': 'Passwörter stimmen nicht überein',
  'incorrect_current_password': 'Falsches aktuelles Passwort',
  'error_changing_password': 'Fehler beim Ändern des Passworts',
  'error': 'Fehler',
  'sign_out': 'Abmelden',
  'random_shake': 'Karten mischen!!',

  // Completion Dialog texts
  'congratulations': 'Glückwunsch!',
  'winner': 'Gewinner: {name}!',
  'its_a_tie': 'Unentschieden!',
  'points_divided': 'Punkte werden bei Unentschieden geteilt!',
  'time_seconds': 'Zeit: {seconds} Sekunden',
  'flips': 'Umdrehungen: {count}',
  'players_score_multiplier': '({players} Spieler: Punkte ×{multiplier})',
  'points_divided_explanation': '(Punkte werden bei Unentschieden geteilt)',
  'health_score': 'Gehirngesundheit Punkte: +{points}',
  'new_game': 'Neues Spiel',
  'times_up': 'Zeit ist abgelaufen!',

  // Tutorial Overlay texts
  'memory_game_guide': 'Gedächtnisspiel Anleitung',
  'card_selection_title': 'Kartenauswahl',
  'card_selection_desc': 'Klicken Sie auf Karten, um Paare zu finden.',
  'time_limit_title': 'Zeitlimit',
  'time_limit_desc':
      'Finden Sie alle Paare vor Ablauf der Zeit. Schnelle Paare bringen mehr Punkte.',
  'add_time_title': 'Zeit hinzufügen',
  'add_time_desc':
      'Klicken Sie auf "+30s", um Zeit hinzuzufügen (kostet Gehirngesundheit Punkte).',
  'multiplayer_title': 'Multiplayer',
  'multiplayer_desc':
      'Ändern Sie die Anzahl der Spieler (1-4), um mit Freunden zu spielen.',
  'dont_show_again': 'Nicht mehr anzeigen',
  'start_game': 'Spiel starten',

  // Brain Health Dashboard texts
  'brain_health_dashboard': 'Gehirngesundheit Dashboard',
  'brain_health_index_title': 'Gehirngesundheit Index',
  'brain_health_index_desc':
      'Verfolgen Sie die Verbesserung Ihres Gehirngesundheit Index durch Gedächtnisspiele. Hohe Level helfen, Demenz vorzubeugen.',
  'activity_graph_title': 'Aktivitätsdiagramm',
  'activity_graph_desc':
      'Sehen Sie, wie sich Ihr Gehirngesundheit Index im Laufe der Zeit im Diagramm ändert.',
  'ranking_system_title': 'Ranglistensystem',
  'ranking_system_desc':
      'Vergleichen Sie Ihren Gehirngesundheit Index mit anderen Benutzern und überprüfen Sie Ihre Position.',
  'game_statistics_title': 'Spielstatistiken',
  'game_statistics_desc':
      'Sehen Sie verschiedene Statistiken wie gespielte Spiele, gefundene Paare und beste Zeiten.',
  'got_it': 'Verstanden!',

  // Brain Health Dashboard additional texts
  'play_memory_games_description':
      'Spielen Sie Gedächtnisspiele, um Ihre Gehirngesundheit zu verbessern!',
  'calculating_brain_health_index': 'Berechnung des Gehirngesundheit Index...',
  'error_calculating_index':
      'Fehler bei der Berechnung des Gehirngesundheit Index',
  'age': 'Alter',
  'update': 'Aktualisieren',
  'points_to_next_level': '{points} Punkte für das nächste Level benötigt',
  'maximum_level_reached': 'Maximales Level erreicht',
  'index_components': 'Index-Komponenten',
  'age_factor': 'Altersfaktor',
  'recent_activity': 'Aktuelle Aktivität',
  'game_performance': 'Spielleistung',
  'persistence_bonus': 'Durchhaltebonus',
  'inactivity_penalty': 'Inaktivitätsstrafe',
  'inactivity_warning':
      'Sie haben {days} Tage nicht gespielt. Ihr Index sinkt täglich!',
  'loading_data': 'Daten werden geladen...',
  'refresh_data': 'Daten aktualisieren',

  // Login Prompt texts
  'start_tracking_brain_health': 'Gehirngesundheit Tracking starten',
  'login_prompt_desc':
      'Melden Sie sich an, um Ihren Gehirngesundheit Index zu verfolgen und Fortschritte zu sehen. Verbessern Sie Ihre kognitiven Fähigkeiten durch Gedächtnisspiele.',
  'sign_in': 'Anmelden',
  'create_account': 'Konto erstellen',

  // User Rankings texts
  'user_rankings': 'Benutzerrangliste',
  'rank': 'Rang',
  'user': 'Benutzer',
  'score': 'Punkte',
  'failed_to_load_rankings': 'Rangliste konnte nicht geladen werden',
  'no_ranking_data': 'Keine Ranglistendaten',

  // Date format texts
  'today': 'Heute',
  'yesterday': 'Gestern',

  // Activity Chart texts
  'brain_health_progress': 'Gehirngesundheit Fortschritt',
  'welcome_to_brain_health': 'Willkommen bei Gehirngesundheit!',
  'start_playing_memory_games':
      'Beginnen Sie mit dem Spielen von Gedächtnisspielen\num Ihre Gehirngesundheit zu verfolgen',
  'date_range': 'Datumsbereich',
  'last_7_days': 'Letzte 7 Tage',
  'last_30_days': 'Letzte 30 Tage',
  'all_time': 'Alle Zeiten',

  // Game Statistics texts
  'game_statistics': 'Spielstatistiken',
  'games_played': 'Gespielte Spiele',
  'matches_found': 'Gefundene Paare',
  'best_times': 'Beste Zeiten',
  'no_records_yet': 'Noch keine Aufzeichnungen',
  'grid': 'Raster',

  // Benefits Section texts
  'benefits_of_brain_games': 'Vorteile von Gehirnspielen',
  'short_term_memory_improvement': 'Verbesserung des Kurzzeitgedächtnisses',
  'short_term_memory_desc':
      'Gedächtnisspiele verbessern effektiv das Kurzzeitgedächtnis und die Gedächtniskapazität.',
  'cognitive_function_enhancement': 'Verbesserung der kognitiven Funktionen',
  'cognitive_function_desc':
      'Regelmäßige Gehirnaktivität hilft, kognitive Funktionen zu schützen und zu verbessern.',
  'response_time_reduction': 'Reduzierung der Reaktionszeit',
  'response_time_desc':
      'Schnelles Matching verbessert Reaktionszeit und Verarbeitungsgeschwindigkeit.',
  'dementia_prevention': 'Demenzprävention',
  'dementia_prevention_desc':
      'Regelmäßiges Gehirntraining hilft, das Risiko von Demenz und kognitivem Verfall zu reduzieren.',

  // Test Page texts
  'memory_test': 'Gedächtnistest',
  'question': 'Frage',
  'visual_memory_test': 'Visueller Gedächtnistest',
  'visual_memory_test_desc':
      'Testen Sie Ihr Gedächtnis mit 10 Fragen. Wählen Sie das Bild, das dem richtigen Wort entspricht.',
  'audio_assistance': 'Audio-Unterstützung',
  'audio_assistance_desc':
      'Klicken Sie auf das Audio-Symbol, um das richtige Wort zu hören. Audio wird in der ausgewählten Sprache abgespielt.',
  'question_navigation': 'Fragennavigation',
  'question_navigation_desc':
      'Verwenden Sie die Zahlenindikatoren oben oder die Pfeiltasten, um zwischen Fragen zu navigieren.',
  'select_and_submit': 'Auswählen und einreichen',
  'select_and_submit_desc':
      'Wählen Sie ein Bild für jede Frage. Die Einreichungsschaltfläche erscheint nach Beantwortung aller Fragen.',
  'results_and_progress': 'Ergebnisse und Fortschritt',
  'results_and_progress_desc':
      'Nach der Einreichung sehen Sie Ihre Ergebnisse und können einen neuen Test starten, wenn Sie möchten.',
  'how_to_play': 'Wie man spielt',
  'start_learning': 'Lernen beginnen',
  'test_result': 'Testergebnis',
  'your_score': 'Ihre Punktzahl',
  'new_test': 'Neuer Test',
  'submit': 'Einreichen'
};
