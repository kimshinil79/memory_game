// Qatar translations (Arabic)
const Map<String, String> arQATranslations = {
  'select_language': 'اختر اللغة',
  'search_language': 'ابحث عن اللغة',
  'all': 'الكل',
  'asian_languages': 'اللغات الآسيوية',
  'european_languages': 'اللغات الأوروبية',
  'middle_eastern_languages': 'لغات الشرق الأوسط',
  'african_languages': 'اللغات الأفريقية',
  'cancel': 'إلغاء',
  'ok': 'موافق',
  'yes': 'نعم',
  'no': 'لا',
  'save': 'حفظ',
  'app_title': 'لعبة الذاكرة',
  'delete': 'حذف',
  'edit': 'تعديل',
  'close': 'إغلاق',
  'back': 'رجوع',
  'next': 'التالي',
  'continue': 'متابعة',
  'settings': 'الإعدادات',
  'push_notifications': 'الإشعارات الفورية',
  'receive_game_notifications': 'تلقي إشعارات اللعبة',
  'profile': 'الملف الشخصي',
  'home': 'الرئيسية',
  'game': 'اللعبة',
  'ranking': 'التصنيف',
  'brain_health': 'صحة الدماغ',
  'player': 'لاعب',
  'players': 'لاعبون',

  // Player Selection Dialog texts
  'select_players': 'اختر اللاعبين',
  'select_up_to_3_players': 'اختر حتى 3 لاعبين',
  'you_will_be_included': 'سيتم إدراجك تلقائياً',
  'confirm': 'تأكيد',
  'retry': 'إعادة المحاولة',
  'no_other_users': 'لم يتم العثور على مستخدمين آخرين',
  'failed_to_load_users': 'فشل في تحميل المستخدمين',
  'country': 'البلد',
  'level': 'المستوى',
  'unknown': 'غير معروف',
  'unknown_player': 'لاعب غير معروف',
  'multiplayer_verification': 'التحقق من اللعب الجماعي',
  'create_pin': 'إنشاء رمز PIN',
  'enter_pin_for': 'أدخل رمز PIN لـ {name}',
  'no_pin_for': 'لا يوجد رمز PIN لـ {name}',
  'create_pin_for_multiplayer': 'إنشاء رمز PIN للعب الجماعي',
  'enter_2_digit_pin': 'أدخل رمز PIN مكون من رقمين',
  'pin_is_2_digits': 'يجب أن يكون رمز PIN مكوناً من رقمين',
  'wrong_pin': 'رمز PIN غير صحيح',

  // Grid Selection Dialog texts
  'select_grid_size': 'اختر حجم الشبكة',
  'choose_difficulty': 'اختر مستوى الصعوبة',
  'multiplier': '×',

  // Profile Edit Dialog texts
  'edit_profile': 'تعديل الملف الشخصي',
  'nickname': 'الاسم المستعار',
  'enter_nickname': 'أدخل اسماً مستعاراً',
  'birthday': 'تاريخ الميلاد',
  'select_birthday': 'اختر تاريخ الميلاد',
  'gender': 'الجنس',
  'male': 'ذكر',
  'female': 'أنثى',
  'select_country': 'اختر البلد',
  'multi_game_pin': 'رمز PIN للعب الجماعي',
  'enter_two_digit_pin': 'أدخل رمز PIN مكون من رقمين',
  'two_digit_pin_helper': 'يجب أن يكون رمز PIN مكوناً من رقمين',
  'change_password': 'تغيير كلمة المرور',
  'current_password': 'كلمة المرور الحالية',
  'enter_current_password': 'أدخل كلمة المرور الحالية',
  'new_password': 'كلمة المرور الجديدة',
  'enter_new_password': 'أدخل كلمة المرور الجديدة',
  'confirm_password': 'تأكيد كلمة المرور',
  'confirm_new_password': 'تأكيد كلمة المرور الجديدة',
  'must_be_two_digit': 'يجب أن يكون مكوناً من رقمين',
  'current_password_required': 'كلمة المرور الحالية مطلوبة',
  'password_length_error': 'يجب أن تكون كلمة المرور مكونة من رقمين',
  'passwords_do_not_match': 'كلمات المرور غير متطابقة',
  'incorrect_current_password': 'كلمة المرور الحالية غير صحيحة',
  'error_changing_password': 'خطأ في تغيير كلمة المرور',
  'error': 'خطأ',
  'sign_out': 'تسجيل الخروج',
  'delete_account': 'حذف الحساب',
  'random_shake': 'هزة عشوائية!',

  // Completion Dialog texts
  'congratulations': 'تهانينا!',
  'winner': 'الفائز: {name}!',
  'its_a_tie': 'تعادل!',
  'points_divided': 'تم تقسيم النقاط!',
  'time_seconds': 'الوقت: {seconds} ثانية',
  'flips': 'القلبات: ',
  'players_score_multiplier': '({players} لاعبين ×{multiplier})',
  'points_divided_explanation': '(تم تقسيم النقاط بين اللاعبين)',
  'health_score': 'صحة +{points}',
  'score_breakdown': 'Score Breakdown',
  'base_score': 'Base Score',
  'streak_bonus': 'Streak Bonus',
  'total_earned': 'Total Earned',
  'new_game': 'لعبة جديدة',
  'share': 'مشاركة',
  'times_up': 'انتهى الوقت!',

  // Tutorial Overlay texts
  'memory_game_guide': 'دليل لعبة الذاكرة',
  'card_selection_title': 'اختيار البطاقات',
  'card_selection_desc': 'اقلب البطاقات للعثور على الأزواج المتطابقة.',
  'time_limit_title': 'الحد الزمني',
  'time_limit_desc': 'أكمل اللعبة قبل انتهاء الوقت.',
  'add_time_title': 'إضافة وقت',
  'add_time_desc': 'اعثر على أزواج متطابقة للحصول على وقت إضافي.',
  'multiplayer_title': 'اللعب الجماعي',
  'multiplayer_desc': 'العب مع الأصدقاء، تنافس للحصول على أعلى درجة.',
  'dont_show_again': 'عدم إظهار مرة أخرى',
  'start_game': 'بدء اللعبة',

  // Brain Health Dashboard texts
  'brain_health_dashboard': 'لوحة صحة الدماغ',
  'brain_health_index_title': 'مؤشر صحة الدماغ',
  'brain_health_index_desc': 'تابع تقدم صحة دماغك.',
  'activity_graph_title': 'رسم بياني للنشاط',
  'activity_graph_desc': 'شاهد نشاطك اليومي في تدريب الدماغ.',
  'ranking_system_title': 'نظام التصنيف',
  'ranking_system_desc': 'قارن أداءك مع اللاعبين الآخرين.',
  'game_statistics_title': 'إحصائيات اللعبة',
  'game_statistics_desc': 'شاهد إحصائيات مفصلة للعبة.',
  'got_it': 'فهمت!',

  // Brain Health Dashboard additional texts
  'play_memory_games_description': 'العب ألعاب الذاكرة لتحسين صحة دماغك.',
  'calculating_brain_health_index': 'جاري حساب مؤشر صحة الدماغ...',
  'error_calculating_index': 'خطأ في حساب المؤشر',
  'age': 'العمر',
  'update': 'تحديث',
  'points_to_next_level': 'النقاط المطلوبة للوصول إلى المستوى التالي: {points}',
  'maximum_level_reached': 'تم الوصول إلى المستوى الأقصى',
  'index_components': 'مكونات المؤشر',
  'age_factor': 'عامل العمر',
  'recent_activity': 'النشاط الأخير',
  'game_performance': 'أداء اللعبة',
  'persistence_bonus': 'مكافأة المثابرة',
  'inactivity_penalty': 'عقوبة عدم النشاط',
  'inactivity_warning': 'تحذير من عدم النشاط: {days} يوم',
  'loading_data': 'جاري تحميل البيانات...',
  'refresh_data': 'تحديث البيانات',

  // Login Prompt texts
  'start_tracking_brain_health': 'ابدأ في تتبع صحة دماغك',
  'login_prompt_desc': 'سجل الدخول لتتبع تقدم صحة دماغك.',
  'sign_in': 'تسجيل الدخول',
  'login': 'Login',
  'create_account': 'إنشاء حساب',

  // User Rankings texts
  'user_rankings': 'تصنيف المستخدمين',
  'rank': 'المرتبة',
  'user': 'المستخدم',
  'score': 'الدرجة',
  'failed_to_load_rankings': 'فشل في تحميل التصنيف',
  'no_ranking_data': 'لا توجد بيانات تصنيف متاحة',

  // Date format texts
  'today': 'اليوم',
  'yesterday': 'الأمس',

  // Activity Chart texts
  'brain_health_progress': 'تقدم صحة الدماغ',
  'welcome_to_brain_health': 'مرحباً بك في صحة الدماغ',
  'start_playing_memory_games': 'ابدأ في لعب ألعاب الذاكرة',
  'date_range': 'نطاق التاريخ',
  'last_7_days': 'آخر 7 أيام',
  'last_30_days': 'آخر 30 يوم',
  'all_time': 'كل الوقت',

  // Game Statistics texts
  'game_statistics': 'إحصائيات اللعبة',
  'games_played': 'الألعاب التي تم لعبها',
  'matches_found': 'الأزواج التي تم العثور عليها',
  'best_times': 'أفضل الأوقات',
  'no_records_yet': 'لا توجد سجلات حتى الآن',
  'grid': 'الشبكة',

  // Benefits Section texts
  'benefits_of_brain_games': 'فوائد ألعاب الدماغ',
  'short_term_memory_improvement': 'تحسين الذاكرة قصيرة المدى',
  'short_term_memory_desc': 'زيادة قدرة ذاكرتك قصيرة المدى.',
  'cognitive_function_enhancement': 'تحسين الوظائف الإدراكية',
  'cognitive_function_desc': 'تحسين الوظائف الدماغية العامة وسرعة المعالجة.',
  'response_time_reduction': 'تقليل وقت الاستجابة',
  'response_time_desc': 'درّب دماغك على الاستجابة بشكل أسرع للمنبهات البصرية.',
  'dementia_prevention': 'الوقاية من الخرف',
  'dementia_prevention_desc':
      'يمكن أن يساعد التدريب المنتظم للدماغ في منع التدهور المعرفي.',

  // Test Page texts
  'memory_test': 'اختبار الذاكرة',
  'question': 'سؤال',
  'visual_memory_test': 'اختبار الذاكرة البصرية',
  'visual_memory_test_desc': 'اختبر قدرتك على تذكر الأنماط البصرية.',
  'audio_assistance': 'المساعدة الصوتية',
  'audio_assistance_desc': 'تفعيل الإشارات الصوتية لتحسين إمكانية الوصول.',
  'question_navigation': 'تنقل الأسئلة',
  'question_navigation_desc': 'التنقل بسهولة بين أسئلة الاختبار.',
  'select_and_submit': 'اختيار وإرسال',
  'select_and_submit_desc': 'اختر إجابتك وأرسل الاختبار.',
  'results_and_progress': 'النتائج والتقدم',
  'results_and_progress_desc': 'شاهد نتائج اختبارك وتقدمك.',
  'how_to_play': 'كيفية اللعب',
  'start_learning': 'ابدأ التعلم',
  'test_result': 'نتيجة الاختبار',
  'your_score': 'درجتك',
  'new_test': 'اختبار جديد',
  'submit': 'إرسال',

  // Brain Level Guide texts
  'brain_level_guide': 'دليل مستويات الدماغ',
  'understand_level_means': 'افهم ما يعنيه مستوى دماغك',
  'rainbow_brain_level5': 'دماغ قوس القزح (المستوى 5)',
  'rainbow_brain_desc': 'دماغك يتلألأ بالبريق الملون!',
  'rainbow_brain_fun': 'لقد وصلت إلى ما يعادل قوس قزح مزدوج في الإدراك - مذهل تماماً!',
  'gold_brain_level4': 'دماغ ذهبي (المستوى 4)',
  'gold_brain_desc': 'وظائف إدراكية وذاكرة ممتازة.',
  'gold_brain_fun': 'ذاكرة خارقة تقريباً - ربما تتذكر أين وضعت مفاتيحك!',
  'silver_brain_level3': 'دماغ فضي (المستوى 3)',
  'silver_brain_desc': 'صحة دماغ جيدة مع مجال للتحسن.',
  'silver_brain_fun': 'دماغك يسخن - مثل كمبيوتر يبدأ في الصباح.',
  'bronze_brain_level2': 'دماغ برونزي (المستوى 2)',
  'bronze_brain_desc': 'وظائف إدراكية متوسطة - نحتاج المزيد من الألعاب!',
  'bronze_brain_fun': 'دماغك نعسان قليلاً - وقت لقهوة ذهنية!',
  'poop_brain_level1': 'دماغ براز (المستوى 1)',
  'poop_brain_desc': 'تبدأ رحلة صحة دماغك.',
  'poop_brain_fun': 'دماغك الآن مثل هاتف ذكي ببطارية 1% - يحتاج شحن عاجل!',
  'keep_playing_memory_games': 'استمر في لعب ألعاب الذاكرة لتحسين صحة دماغك!',
  // Ranking tabs
  'total': 'المجموع',
  'weekly': 'أسبوعي',
  'monthly': 'شهري',

  // Promotional messages
  'promo_message_1': 'ابدأ تطوير دماغ طفلك من خلال اللعب! 🎨 ألعاب ممتعة تعزز مهارات الذاكرة!',
  'promo_message_2': 'مثالية لتركيز طفلك! ✨ ابنِ مهارات التركيز والملاحظة في جلسات قصيرة.',
  'promo_message_3': 'عزز كفاءة التعلم بألعاب البوب أب! 🚀 درب ذاكرتك، أساس الدراسة.',
  'promo_message_4': 'هل يمكنك الانتهاء أسرع من أصدقائك؟ 👀 اختبر الإنجاز وتنشيط الدماغ من خلال التحديات!',
  'promo_message_5': 'فيتامين الدماغ خلال موسم الامتحانات! 🧠 عزز التركيز والذاكرة بجلسات تدريب سريعة!',
  'promo_message_6': 'وقت استراحة سريع! 💫 خفف التوتر بينما تعيد شحن دماغك.',
  'promo_message_7': 'دماغ متعب من العمل الإضافي؟ 💡 عزز التركيز في العمل بـ 5 دقائق فقط في اليوم.',
  'promo_message_8': 'اهرب من إدمان الهاتف الذكي! 📵 استبدل التمرير بلا هدف بألعاب تعزز الدماغ.',
  'promo_message_9': 'عزز ذاكرة الوالدين! ✅ إبهام للأعلى للوقاية من الخرف! التمرين المستمر للدماغ هو الحل.',
  'promo_message_10': 'وداعاً للنسيان! 👋 حافظ على دماغ شاب بألعاب الذاكرة الممتعة!',
  // Top Fastest by Grid

  'top_fastest_by_grid': 'الأسرع حسب الشبكة',     
  // Player Selection Dialog
  'search_players': 'البحث عن اللاعبين...',
  'solo_play': 'لعب منفرد',
  'no_search_results': 'لم يتم العثور على لاعبين',
  'try_different_search': 'جرب مصطلح بحث مختلف',
'by_country': 'حسب البلد',

  // Login Prompt Messages (for game completion)
  'login_prompt_1': '🎯 احفظ نتائجك وتنافس في التصنيفات العالمية!',
  'login_prompt_2': '🧠 تتبع نقاط صحة دماغك وراقب تقدمك!',
  'login_prompt_3': '🏆 تحدى أصدقائك بألعاب متعددة اللاعبين!',
  'login_prompt_4': '📊 تحقق من إحصائيات لعبتك وحسّن مهاراتك!',
  'login_prompt_5': '⭐ سجل لفتح المزيد من الميزات الرائعة!',
  'login_prompt_6': '🎮 عزز صحة الدماغ مع أصدقائك!',
  'login_prompt_7': '📈 حطم الأرقام القياسية الشخصية وطور قدراتك!',

};